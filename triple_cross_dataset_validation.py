# -*- coding: utf-8 -*-
"""
Triple Cross-Dataset Validation Script
Train on MulticlassDiabetesDataset and test on:
1. Mapped Pima dataset (direct mapping)
2. Synthetic dataset generated from Pima (using SMOTE)
3. CDC Diabetes Health Indicators dataset (real multiclass dataset)
"""
...
# Import CDC dataset integration
from cdc_dataset_integration import load_cdc_dataset
...
def load_and_map_pima_dataset():
    ...
    mapped_df['Gender'] = 1  # Pima dataset is all females
    
    # Map glucose level
    mapped_df['HbA1c'] = pima_df['Glucose'] / 40.0
    
    # Use available features as proxies
    ...
    # Create synthetic lipid profile
...
def generate_synthetic_dataset_from_pima(X_mapped, y_pima, target_samples=500):
    """Generate synthetic dataset from mapped Pima data using SMOTE"""
    print(f"\nGenerating synthetic dataset from Pima data...")
    
    # Use SMOTE to balance and expand the dataset
...
def train_models(X_train, y_train):
    ...
    rf_params = {'n_estimators': [100, 200], 'max_depth': [3, 5], 'min_samples_split': [2, 5]}
    ...
    return {'scaler': scaler, 'lr': lr_best, 'rf': rf_best, 'xgb': xgb_best, 'ensemble': voting_clf}
...
def main():
    ...
    print("="*80)
    print("TRIPLE CROSS-DATASET DIABETES PREDICTION VALIDATION")
    print("Training on: MulticlassDiabetesDataset")
    print("Testing on: 1) Mapped Pima  2) Synthetic Pima  3) CDC Dataset")
    print("="*80)
    ...
    X_pima_mapped = X_pima_mapped[feature_columns]
    ...
    print(f"Test 2 (Synthetic Pima): {X_synthetic.shape}")
    print(f"Test 3 (CDC Dataset): {X_cdc.shape}")
    ...
    synthetic_results = evaluate_models(models, X_synthetic, y_synthetic, "Synthetic Pima Dataset")
    cdc_results = evaluate_models(models, X_cdc, y_cdc, "CDC Diabetes Dataset")
    ...
    print(f"Test Dataset 2: Synthetic Pima Dataset ({X_synthetic.shape[0]} samples)")
    print(f"Test Dataset 3: CDC Diabetes Dataset ({X_cdc.shape[0]} samples)")
    
    print(f"\nCross-Dataset Performance Comparison:")
    print(f"{'Model':<12} {'Pima':<8} {'Synthetic':<10} {'CDC':<8}")
    print("-" * 40)
    
    for model_name in ['lr', 'rf', 'xgb', 'ensemble']:
        pima_acc = pima_results.get(model_name, 0)
        synth_acc = synthetic_results.get(model_name, 0)
        cdc_acc = cdc_results.get(model_name, 0)
        print(f"{model_name.capitalize():<12} {pima_acc:<8.4f} {synth_acc:<10.4f} {cdc_acc:<8.4f}")
    
    print(f"\nKey Insights:")
    print(f"- Training accuracy: {train_accuracy:.4f} (expected to be high)")
    print(f"- Cross-dataset performance varies significantly across datasets")
    print(f"- This demonstrates real-world ML generalization challenges")
    print(f"- Multiple test datasets provide robust validation")
    
    # Best performing approach
    ensemble_scores = [pima_results['ensemble'], synthetic_results['ensemble'], cdc_results['ensemble']]
    best_dataset_idx = ensemble_scores.index(max(ensemble_scores))
    dataset_names = ['Mapped Pima', 'Synthetic Pima', 'CDC Dataset']
    
    print(f"\nBest Cross-Dataset Performance:")
    print(f"- {dataset_names[best_dataset_idx]}: {max(ensemble_scores):.4f} (Ensemble)")
    print(f"- This suggests the most suitable approach for your domain")
